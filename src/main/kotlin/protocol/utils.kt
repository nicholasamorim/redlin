 ...
 */
internal fun serializeContainer(data: Any?): ByteArray = when (data) {
    is String -> when (USE_BULK_STRING) {
    ...
    is List<*> -> when (data.isEmpty()) {
        ...
        false -> {
            ...
            collect.fold("${ArrayType.firstByte}${collect.size}$CRLF".toByteArray(), ByteArray::plus)
        }
    }
...
internal fun <T : MutableCollection<Any?>> deserializeArray(data: ByteArray, container: T): Pair<T, Int> {
